import { GamePhase, BuffType, } from './types';
...
// ===== 游戏创建 ===== 
export function createGame(roomId, p1Id, p1Name, p2Id, p2Name) {
    return {
        roomId,
        players: [
            {
                ...
                deck: shuffleDeck({ deck: buildTestDeck(), hand: [], discardPile: [], buffs: [], equipment: {} }).deck,
                hand: [],
                discardPile: [],
                buffs: [],
                equipment: {},
...
export function playCard(state, action, playerId) {
    // 校验游戏状态 
...
export function handleDiscardBuffs(player, s) {
    // 绑定诅咒：丢弃牌时受伤害 
...
// ===== 丢弃手牌 ===== 
export function discardFromHand(state, playerId, cardId, targetId) {
    let s = deepClone(state);
...
// ===== 卸下装备 ===== 
export function unequipCard(state, playerId, slot) {
    const s = deepClone(state);
    const idx = s.players.findIndex(p => p.id === playerId);
    if (idx === -1)
        return s;
...
// ===== 侦测器：处理权重猜测 ===== 
export function handleGuessWeight(state, playerId, guessWeight) {
    const s = deepClone(state);
    const idx = s.players.findIndex(p => p.id === playerId);
    if (idx === -1)
        return s;
    ...
    if (correct)
        applyEffectToPlayer(player, BuffType.DamageBoost, 1, 1, 'detector', player.id);
...
// ===== 运输矿车：处理选牌 ===== 
export function handleDraftPick(state, playerId, cardIndex) {
    const s = deepClone(state);
    ...
    if (!owner.draftCards || owner.draftCards.length === 0)
        return s;
    // 判断该轮到谁选 
    const isOwnerPick = owner.id === playerId;
    const expectedPick = isOwnerPick ? 0 : 1;
    if (owner.draftPlayerPick !== expectedPick)
        return s;
    if (cardIndex < 0 || cardIndex >= owner.draftCards.length)
        return s;
    if (owner.draftPickedBy && owner.draftPickedBy[cardIndex])
        return s;
    // 牌给当前选牌的玩家 
    const picked = owner.draftCards[cardIndex];
    addCardToHand(s.players[pickerIdx], picked);
    owner.draftPickCount += 1;
    if (!owner.draftPickedBy)
        owner.draftPickedBy = {};
...
// ===== 蜘蛛网：处理封锁选择 ===== 
export function handleBucketChoice(state, playerId, lockType) {
    const s = deepClone(state);
    const idx = s.players.findIndex(p => p.id === playerId);
    if (idx === -1)
        return s;
    const player = s.players[idx];
    if (player.pendingBucketChoice !== 'pending')
        return s;
    const oppIdx = 1 - idx;
...
// ===== 诡异钓竿：处理装备丢弃 ===== 
export function handleEquipChoice(state, playerId, slot) {
    let s = deepClone(state);
    const idx = s.players.findIndex(p => p.id === playerId);
    if (idx === -1)
        return s;
    const player = s.players[idx];
    if (player.pendingEquipChoice !== 'pending')
        return s;
    const oppIdx = 1 - idx;
    const opponent = s.players[oppIdx];
    const slotKey = slot;
...
// ===== 诡异钓竿：取消选择，返还卡牌 =====
export function cancelEquipChoice(state, playerId) {
    let s = deepClone(state);
    const idx = s.players.findIndex(p => p.id === playerId);
    if (idx === -1)
        return s;
...
// ===== 酿造台：处理卡牌转化 ===== 
export function handleBrewConversion(state, playerId, cardId) {
    const s = deepClone(state);
    const idx = s.players.findIndex(p => p.id === playerId);
    if (idx === -1)
        return s;
    const player = s.players[idx];
    if (player.equipment?.weapon?.name !== '酿造台')
        return s;
    const cardIdx = player.hand.findIndex(c => c.id === cardId);
    if (cardIdx === -1)
        return s;
    ...
    if (card.name === '苹果')
        targetName = '烟花';
    else if (card.name === '烟花')
        targetName = '苹果';
    // 新增功能：龙息 <-> 金苹果 
    else if (card.name === '龙息')
        targetName = '金苹果';
    else if (card.name === '金苹果')
        targetName = '龙息';
    else
        return s;
