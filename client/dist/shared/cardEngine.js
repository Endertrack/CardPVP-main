...
export function showMessage(msg, target = 'all', category = 'hint') {
    const h = globalThis.__card_notify_handler;
...
export function heal(source, target, number, opponent, state) {
...
(function (DamageType) {
    ...
    DamageType[DamageType["Real"] = 2] = "Real";
})(DamageType || (DamageType = {}));
/** 原地消耗 buff 层数（修改原对象 buffs 数组，不创建新对象） */
export function consumeInPlace(player, type, amount) {
    let remaining = amount;
    for (const b of player.buffs) {
        if (b.buffType !== type || remaining <= 0)
            continue;
...
export function damage(source, target, type, base, isCard) {
...
export function applyCard(gameState, playerId, targetId, card) {
    ...
    if (subtype === 'heal') {
        if (p.equipment?.field?.name === '冰原' && (p.healCountThisTurn || 0) >= 1) {
            ...
            p.healCountThisTurn = (p.healCountThisTurn || 0) + 1;
    }
    if (subtype === 'attack') {
        if (p.equipment?.field?.name === '冰原' && (p.attackCountThisTurn || 0) >= 1) {
            ...
            p.attackCountThisTurn = (p.attackCountThisTurn || 0) + 1;
    }
    ...
    for (const effect of card.effects) {
        ...
        if (effect.buffType === BuffType.Heal) {
            ...
            if (effect.duration && effect.duration > 0) {
                ...
                damage(p, target, DamageType.Real, effect.value, true);
        }
            ...
            // 强制丢弃装备/武器/场地
            const target = isSelfTarget ? p : t;
            const slots = ['equip', 'weapon', 'field'];
    ...
    const hpParts = [];
    if (newHpP !== oldHpP)
        hpParts.push(`自己血量${oldHpP}→${newHpP}`);
    if (!isSelfTarget && newHpT !== oldHpT)
        hpParts.push(`对方血量${oldHpT}→${newHpT}`);
